...
const TimeRangeSelectorComponent = ({
  className,
  defaultValue = [2011, 2021],
  value,
  min = 2011,
  max = 2021,
  onValueChange,
  ...
  return (
    <div className="w-full rounded-md border border-input bg-transparent flex justify-center px-[13px] gap-4 shadow-sm">
      <span className="text-sm text-muted-foreground border-r-2 min-w-24 py-[7px] font-semibold">
        {`${currentValue[0]} - ${currentValue[1]}`}
      </span>
      <SliderPrimitive.Root
        data-slot="slider"
        defaultValue={Array.isArray(defaultValue) ? defaultValue : [min, max]}
        value={currentValue}
